      ...
      $facet: {
        ...
        products: [
          ...
          // },
          {
            $addFields: {
              product_price: {
                $toDouble: "$product_price",
              },
              b2b_user_product_price: {
                $toDouble: "$b2b_user_product_price",
              },
              b2c_user_product_price: {
                $toDouble: "$b2c_user_product_price",
              },
              product_collected_points: {
                $toDouble: "$product_collected_points",
              },
              product_reward_points: {
                $toDouble: "$product_reward_points",
              },
            },
          },
        ],
...
const cartCheckout = catchAsync(async (req, res, next) => {
  ...
  let order = {
    ...
    product: {
      ...
      quantityBought: 1,
      b2b_user_product_price: product.b2b_user_product_price
        ? parseFloat(product.b2b_user_product_price)
        : null,
      b2c_user_product_price: product.b2c_user_product_price
        ? parseFloat(product.b2c_user_product_price)
        : null,
...
/**
 * @author  Sam
 * @route   all/reward/products/history
 * @method  POST
 * @access  Protected
 * @desc    Place a reward order.
 */
const getRewardOrderHistoryByUser = catchAsync(async (req, res, next) => {
  const user = req.user;
  const page = parseInt(req.query.page) || 1;
  const pageSize = parseInt(req.query.pageSize) || 10;

  const totalResults = await RewardOrder.find({ buyer: user._id }).count();

  const orders = await RewardOrder.find({ buyer: user._id })
    .sort({
      createdAt: -1,
    })
  ...
  const formattedOrders = orders.map((orderItem) => {
    const order = orderItem.toObject();

    orderItem.b2c_user_product_price;
    return {
      ...order,
      product: {
        ...order.product,
        b2c_user_product_price: parseFloat(
          order.product.b2c_user_product_price.toString()
        ),
        b2b_user_product_price: parseFloat(
          order.product.b2b_user_product_price.toString()
        ),
      },
    };
  });
...
module.exports = {
  getRewardPointsRedeemableProducts,
  cartCheckout,
  getRewardOrderHistoryByUser,
};
