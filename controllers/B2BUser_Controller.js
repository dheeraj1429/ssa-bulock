...
const getAllB2bUsers = catchAsync(async (req, res, next) => {
  ...
  const allUsers = await B2BUser.find(
    {},
    { is_approved: 1, company_name: 1, owner_name: 1, email: 1, mobile: 1 }
  )
...
const getSingleB2bUser = catchAsync(async (req, res, next) => {
  ...
  const findUserDocuments = await B2BUser.findOne(
    { _id: userId },
...
const setDealerCode = catchAsync(async (req, res, next) => {
  const { userId, code } = req.body;
  if (!isValidObjectId(userId))
    throw new CaptureError("Invalid B2B user ID", httpStatus.BAD_REQUEST);

  const checkDealerCodeExists = await B2BUser.findOne({
    dealerCode: code,
    _id: { $ne: userId },
  });

  if (checkDealerCodeExists) {
  ...
  const update = await B2BUser.updateOne(
    { _id: userId },
    { $set: { dealerCode: code } }
  );
  ...
  if (update.modifiedCount) {
    return res.status(200).json({
      success: true,
      message: "B2B User dealer code updated!",
    });
  }
