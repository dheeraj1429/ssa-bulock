...
const verifyUpdatePassword = async (payload) => {
  try {
    const validationResult = await userPasswordSchema.validate(payload, {
      stripUnknown: true,
    });
...
const b2bUserCreateSchema = yup.object({
  ...
  owner_name: yup.string().max(64).required().label('Owner Name'),
  email: yup
    .string()
